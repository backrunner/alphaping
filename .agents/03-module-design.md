...
软删除 finalizer 使用配置的 `soft_delete_grace_days`。机器和服务先按 resource PK 分批清空 block/latest/rollup/event 和 Agent replay，再删除关联授权、检查 secret 与 CONTROL_DB 记录；workspace 删除将未单独标记的子资源视为同时删除，只有两库都无业务行后才物理删除 workspace。跨 D1 不宣称原子性，恢复只允许在 grace window 内完成。
