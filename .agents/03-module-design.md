...
- 使用 `UPDATE ... WHERE config_revision = ? AND last_claimed_slot < ?` 原子领取 due task；执行后再次校验 revision/claim，deterministic execution ID 绑定 check、config revision 和 nominal slot。
...
- 结果以 D1 batch 写入 `check_result_blocks_5m` slot、`check_latest`、已闭合 rollup 和状态事件。中央与 Agent 路径都在 check latest 条件写之后扫描一次当前服务 latest 集合，先插入幂等 transition event，再仅由本批实际插入的 event 通过复合主键更新 `service_latest`；禁止在事务外读取其他 check 后提交预计算服务状态，也禁止被忽略的重复 event 重放旧转换。
- Web 的 check target/policy/delete/maintenance mutation 通过 `packages/db` 共享 repository 写入 `service_state_sync_jobs` 并立即尝试同步；Checks Worker 在本轮检查落库后按 `next_attempt_at,last_attempted_at,job_key` 处理最多 50 个 due job。中央检查 latest 保存 `config_revision`，job 删除旧 revision 但保留当前 revision；job 至少保留 15 分钟并按当前 CONTROL_DB 状态重复校正，每次尝试后以 sync token 条件轮转，避免保护窗内的旧 job 饿死后续 mutation。长维护 job 在保护窗成功收敛后把 `next_attempt_at` 推进到窗口结束，届时重算并删除；失败 job 保持立即可重试。
