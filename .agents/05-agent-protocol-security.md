...
配置 snapshot 包含 revision、created_at、可选采样/上报周期、可选容器监控开关、完整内容 BLAKE3 digest 和最多 32 个结构化 probe task。新增 collection 字段保持 protobuf optional：旧 Agent 忽略未知字段，新 Agent 收到旧服务端省略的字段时继续使用本地 last-known-good 值。Secret 只在 Ingest 内存中从 CONTROL_DB 解密，再放入 s2c AEAD 明文；日志、D1 telemetry 和 Web 投影都不得包含 secret value。Agent 先验证 digest、任务归属字段、周期、timeout 和 payload 上限，再原子保存本地配置并以 SQLite transaction 替换 last-known-good snapshot，在后续 report 回报 applied revision。
