...
async function acquirePrefixLease(
  db: D1Database,
  ...
  return db
    ...
    .bind(prefix, now + LEASE_MS, now, now)
    .first<CursorRow>();
}

async function releasePrefixLease(
  db: D1Database,
  prefix: ArtifactPrefix,
  lastKey: string,
  now: number,
...
async function cleanPrefix(
  ...
  const listed = await env.EXPORT_BUCKET.list({
    prefix,
    ...(cursor.last_key ? { startAfter: cursor.last_key } : {}),
    limit,
  ...
  await releasePrefixLease(env.TELEMETRY_DB, prefix, nextKey, now);
