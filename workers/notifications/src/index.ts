...
export async function runScheduled(env: Env): Promise<void> {
  ...
  if (discovered?.status === "fulfilled" && (discovered.value.scanned > 0 || discovered.value.bootstrapped)) {
  ...
  const failures = Number(discovered?.status === "rejected") + Number(delivered?.status === "rejected");
